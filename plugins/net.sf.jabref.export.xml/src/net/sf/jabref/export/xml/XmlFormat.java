...
public class XmlFormat extends ExportFormat {
  ...
  /**
   * Creates an instance-level parser with utf-8 encoding. The encoding will be
   * overridden by the {@code encoding} parameter in
  ...
  public void performExport(BibtexDatabase database, Writer writer,
    ...
    if (entryIds == null || entryIds.isEmpty()){
      ss = FileActions.saveDatabase(database, null, tmpFile, Globals.prefs,
          false, false, encoding);
    }else{
      List<BibtexEntry> sortedEntries = FileActions.getSortedEntries(database,
          entryIds, true);
    ...
    if (!tempWriter.couldEncodeAll()){
      LOG.warn(ss.getEncoding() + ": " + tempWriter.getProblemCharacters());
    }
    ss.commit();
    ...
    org.w3c.dom.Document prepared = prepare(new DOMOutputter().output(document));

    Format format = Format.getPrettyFormat().setEncoding(encoding);
    new XMLOutputter(format).output(new DOMBuilder().build(prepared), writer);
  }
  ...
  public void performExport(BibtexDatabase database, String file,
      String encoding, Set<String> entryIds) throws Exception {
